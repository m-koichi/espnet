...
from egs2.dcase20t4.sed1.pyscripts.sed.utils.evaluation_measures import compute_psds_from_operating_points
from egs2.dcase20t4.sed1.pyscripts.sed.utils.evaluation_measures import compute_metrics
from egs2.dcase20t4.sed1.pyscripts.sed.utils.evaluation_measures import psds_score
...
class ScoreDataset(Dataset):
        ...
        with h5py.File(score_h5_path, 'r') as h5:
            ...
            for data_id in self.data_ids:
                ...
                if self.has_label:
                    ...
                    self.dataset[data_id] = dict(pred_strong=pred_strong, pred_weak=pred_weak,)
    ...
    def __getitem__(self, index):
        ...
        if self.has_label:
            ...
            return dict(data_id=data_id, pred_strong=pred_strong, pred_weak=pred_weak, target=target)
        else:
            return dict(data_id=data_id, pred_strong=pred_strong, pred_weak=pred_weak,)
...
class PostProcess():
    def __init__(self, model, iterator, output_dir, options):
        self.model = model
    ...
    def get_posterior(self, save_h5_path) -> None:
        with h5py.File(save_h5_path, 'w') as h5:
            ...
            for (data_ids, batch) in self.iterator:
                assert isinstance(batch, dict), type(batch)

                batch = to_device(batch, "cuda" if self.options.ngpu > 0 else "cpu")
                audio, audio_lengths, label, label_lengths \
                    = batch['audio'], batch['audio_lengths'], batch['label'], batch['label_lengths']

                # 1. Extract feats
                feats, feats_lengths = self.model._extract_feats(audio, audio_lengths)

                # 3. Normalization for feature: e.g. Global-CMVN, Utterance-CMVN
                if self.model.normalize is not None:
                    feats, feats_lengths = self.model.normalize(feats, feats_lengths)

                batch['audio'], batch['audio_lengths'] = feats, feats_lengths
                predicts, loss, stats, weight = self.model(**batch)
                predicts["strong"] = torch.sigmoid(predicts["strong"]).cpu().data.numpy()
                predicts["weak"] = torch.sigmoid(predicts["weak"]).cpu().data.numpy()
                # breakpoint()
                for pred_strong, pred_weak, label, data_id in zip(predicts["strong"], predicts["weak"], batch["label"], data_ids):
                    ...
                    ptr = self.pooling_time_ratio
                    h5[data_id].create_dataset('target', data=label.cpu().numpy()[ptr//2::ptr, :])


    def get_prediction_dataframe(self, post_processing=None,
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            if binarization_type == 'class_threshold':
                for i in range(output["strong"].shape[0]):
                    output["strong"][i] = ProbabilityEncoder().binarization(output["strong"][i], binarization_type=binarization_type,
                                                                    threshold=threshold, time_axis=0)
            elif binarization_type == 'global_threshold':
                output["strong"] = ProbabilityEncoder().binarization(output["strong"],
                                                                binarization_type=binarization_type,
                                                                threshold=threshold)
            else:
                raise ValueError("binarization_type must be 'class_threshold' or 'global_threshold'")
            weak = ProbabilityEncoder().binarization(output["weak"], binarization_type='global_threshold',
                                                            threshold=0.5)
    ...
    def search_best_threshold(self, step, target="Event"):
        ...
        for th in np.arange(step, 1.0, step):
            ...
            prediction_df = self.get_prediction_dataframe(threshold=th, binarization_type="global_threshold",
                                                             save_predictions=None)
            events_metric, segments_metric, psds_m_f1 = \
                compute_metrics(prediction_df, self.validation_df, self.durations_validation)
        ...
        prediction_df = self.get_prediction_dataframe(post_processing=None,
    ...
    def search_best_median(self, spans, best_th=None, target='Event'):
        ...
        best_f1 = {k: 0. for k in self.labels}
    ...
    def show_best(self, pp_params, save_predictions=None):
        ...
        prediction_df = self.get_prediction_dataframe(post_processing=post_processing_fn,
                                        threshold=threshold,
                                        binarization_type='class_threshold',)
    ...
    def get_predictions(self, thresholds, post_processing=None, save_predictions=None):
        ...
        for batch_idx, data in enumerate(self.data_loader):
            ...
            for j, (pred, data_id) in enumerate(zip(output["strong"], data['data_id'])):
                for threshold in thresholds:
                    breakpoint()
                    pred = ProbabilityEncoder().binarization(pred,
                                                            binarization_type="global_threshold",
                                                            threshold=threshold)
                    # Apply post processing if exists
                    if post_processing is not None:
    ...
    def tune_all(self,):
        ...
        best_fs, best_f1 = self.search_best_median(
            spans=list(range(1, 31, 2)),
            best_th=best_th
        )

        pp_params = {"threshold": best_th,
