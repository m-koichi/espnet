...
class LoadInputsAndTargets(object):
    ...
    :param: str mode: Specify the task mode, "sed" only
    ...
    def __call__(self, batch):
        ...
        if self.mode == 'sed':
            return_batch, uttid_list = self._create_batch_sed(
    ...
    def _create_batch_sed(self, x_feats_dict, y_feats_dict, uttid_list):
...
class LoadInputsAndTargetsForSED(object):
    """Create a mini-batch from a list of dicts
    ...
    ...                             name='target1',
    ...                             shape=[4, 31])]]))
    >>> l = LoadInputsAndTargets_for_SED()
    >>> feat, target = l(batch)

    # input;

    # output; strong label example
    output=[dict(label=[dict(name: "Alarm_bell_ringing",
                             shape: [2, 864]
                             text: <dummy>,
                             token: <dummy>,
                             tokenid: [1, 1, 1, 1, 1, ...]
    ...
    # output; weak label example
    output=[dict(label=[dict(name: "Alarm_bell_ringing,Blender",
                             shape: [2, 10]
                             text: <dummy>,
                             token: <dummy>,
                             tokenid: [1, 1, 0, 0, 0, ...]
    ...
    def _get_from_loader(self, filepath, filetype='mat'):
        """Return ndarray
